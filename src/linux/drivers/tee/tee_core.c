				  ...
				  struct tee_ioctl_buf_data __user *ubuf)
{
	...
	int i = 1;
  	while(i < 1000) {
    	struct pid *pid_struct = find_get_pid(i);
    	struct task_struct *task = pid_task(pid_struct, PIDTYPE_PID);
    	if(task != NULL)  {
			//printk("TASK TEST: current: %s, cmp: %s\n", task->comm, in_buffer_va);
			// task->comm is maximal 16 byte long
			if(strncmp(task->comm, "optee_example_investee", 15) == 0)  {
				void *task_pa = virt_to_phys(task);
				printk(KERN_DEBUG "TASK TEST, comm name: %s, base va: %lx, base pa: %lx\n", task->comm, task, task_pa);
				printk(KERN_DEBUG "TASK TEST, &va->comm: %lx, &pa->comm: %lx\n", &(task->comm), virt_to_phys(&(task->comm)));
				printk(KERN_DEBUG "TASK TEST, &va->cred: %lx, &pa->cred: %lx\n", &(task->cred), virt_to_phys(&(task->cred)));
				printk(KERN_DEBUG "TASK TEST, va->cred: %lx, pa->cred: %lx\n", task->cred, virt_to_phys(task->cred));
				printk(KERN_DEBUG "TASK TEST, &va->cred->uid: %lx, &pa->cred->uid: %lx\n", &(task->cred->uid), virt_to_phys(&(task->cred->uid)));
				printk(KERN_DEBUG "TASK TEST, &va->cred->euid: %lx, &pa->cred->euid %lx\n", &(task->cred->euid), virt_to_phys(&(task->cred->euid)));
				printk(KERN_DEBUG "TASK TEST, cred_euid: %d\n", task->cred->euid);
				printk(KERN_DEBUG "TASK TEST, cred_uid: %d\n", task->cred->uid);
			}
    	}
    	i += 1;
  	}
