...
static uint32_t std_entry_with_parg(paddr_t parg, bool with_rpc_arg)
{
	...
	if (core_pbuf_is(CORE_MEM_NSEC_SHM, parg, sz)) {
		//IMSG("INVESTEE_LOG: std_entry_with_parg: in shared memory phys_addr: 0x%"PRIxPA, parg);
		...
		//IMSG("INVESTEE_LOG: std_entry_with_parg: optee_msg_arg struct at phys_addr: 0x%"PRIxPA, parg);
...
static uint32_t std_entry_with_regd_arg(uint64_t cookie, size_t offset)
{
	...
	if (!rv)
		IMSG("INVESTEE_LOG: optee_msg_arg->cmd: %lx", arg->cmd);
		IMSG("INVESTEE_LOG: optee_msg_arg->func: %lx", arg->func);
		IMSG("INVESTEE_LOG: optee_msg_arg->session: %lx", arg->session);
		IMSG("INVESTEE_LOG: optee_msg_arg->cancel_id: %lx", arg->cancel_id);
		IMSG("INVESTEE_LOG: optee_msg_arg->pad: %lx", arg->pad);
		IMSG("INVESTEE_LOG: optee_msg_arg->ret: %lx", arg->ret);
		IMSG("INVESTEE_LOG: optee_msg_arg->ret_origin: %lx", arg->ret_origin);
		IMSG("INVESTEE_LOG: optee_msg_arg->num_params: %lx", arg->num_params);
